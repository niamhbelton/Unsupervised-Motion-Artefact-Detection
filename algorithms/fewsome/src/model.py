...
class CIFAR_VGG3_stan(nn.Module):
  def __init__(self,vector_size, biases):
      super(CIFAR_VGG3_stan, self).__init__()
      ...
      self.block7=models.vgg16().features[14]
      self.block8=models.vgg16().features[16]
      self.classifier = nn.Linear(4096, vector_size)
      self.classifier2 = nn.Linear(vector_size, 1)
...
class CIFAR_VGG3_classifier(nn.Module):
  def __init__(self,vector_size, biases):
      super(CIFAR_VGG3_classifier, self).__init__()
  ...
  def forward_eval(self, x1, x2):
        ...
        x = x1-x2 #a vector of differences, model learns to output large differences for, creating representations for how different the test image is from normal images
        x = self.classifier2(x)
    #    x=nn.Sigmoid()(x)
        return x #output
