...
class DeepSVDDTrainer_cmr(BaseTrainer):
    ...
    def test(self, dataset: BaseADDataset, net: BaseNet, eval_epoch =0, epoch=None):
        ...
        dict_score2=dict_score1.copy()
        for key in dict_score1:
                dict_score1[key] = np.mean(dict_score1[key])
                print(dict_score1[key])


        for key in dict_score2:
                dict_score2[key] = np.max(dict_score2[key])

        print(dict_score1)
        score1 = np.array(dict_score1.values())
        score2 = np.array(dict_score2.values())
    #    s1 = pd.DataFrame({'file' : np.array(dict_score1.keys()), 's1':score1})
    #    s2 = pd.DataFrame({'file' : np.array(dict_score2.keys()), 's2':score2})
        print('score 1 {}'.fromat(score1))
        s1 = pd.DataFrame([score1], index = np.array(dict_score2.keys())).reset_index()
    #    s1 = pd.concat([pd.DataFrame(np.array(dict_score2.keys())) , pd.DataFrame(score1)], axis =0).T
        print(s1)
 ...
 #get auc and f1 based on score 1, binary
        ...
        #get auc and f1 based on score 1, severity
        ...
        accuracy_sev = (cl0+cl1+cl2) / len(scores1)
        print('Accuracy with severity {}'.format(accuracy_sev))

        #get auc and f1 based on score 2, severity
        ...
        cl1 = len(outputs[(outputs == 1 ) & (label_sev == 1)])
        cl2 = len(outputs[(outputs == 2 ) & (label_sev == 2)])
        accuracy_sev2 = (cl0+cl1+cl2) / len(scores2)
        print('Accuracy with max{}'.format(accuracy_sev2))
        ...
        logger.info('Severity accuracy based on max anomaly score per volume:  {:.2f}%'.format(100. * accuracy_sev ))
        logger.info('Severity accuracy based on max anomaly score per volume:: {:.2f}%'.format(100. * accuracy_sev2 ))
