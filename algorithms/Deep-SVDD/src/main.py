...
def main(dataset_name, net_name, xp_path, data_path, pollution, n, load_config, load_model, objective, nu, device, seed,
         ...
         ae_n_epochs, ae_lr_milestone, ae_batch_size, ae_weight_decay, n_jobs_dataloader, early_stopping_loss, normal_class, eval_epoch,data_split_path):
    """
    ...
    if dataset_name == 'mvtec':
        deep_SVDD = DeepSVDD_mvtec(cfg.settings['objective'], cfg.settings['nu'])
    ...
    string=xp_path + '/final_data_n_' + str(n) + '_seed_' + str(seed)
    print(deep_SVDD.results)
    deep_SVDD.results['final_data'].to_csv(string)
