...
def load_train(train_path, sample_rate, label_class):
...
#                                                     shuffle=True,
#                                                     drop_last=False)
    train_data_loader, _ = utils.get_loaders(dataset='cifar10', label_class=label_class, batch_size=BATCH_SIZE)
    return train_data_loader, 30 #imagenet_data.__len__()


def load_test(test_path, label_class, sample_rate):
    transform = torchvision.transforms.Compose([
        torchvision.transforms.Resize(256),
        torchvision.transforms.CenterCrop(256),
        torchvision.transforms.ToTensor(),
...
#                                                     pin_memory=True,
#                                                     shuffle=False)
    _, valid_data_loader = utils.get_loaders(dataset='cifar10', label_class=label_class, batch_size=BATCH_SIZE)
...
def train(args, NORMAL_NUM,
    ...
    train_dataset_loader, train_size = load_train(train_path, args.sample_rate, NORMAL_NUM)
...
def validation(NORMAL_NUM, iteration, generator, discriminator, real_data, fake_data, is_end, AUC_LIST, END_ITER):
    ...
    with torch.no_grad():
        for i in range(1): # range(len(list_test)): #
            ...
            valid_dataset_loader = load_test(test_path, NORMAL_NUM, sample_rate=1.)
