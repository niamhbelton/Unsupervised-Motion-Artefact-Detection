...
MAX_EPOCH = 20 #128 #256 #2 #10 # 
num_worker = 6
############################ Parameters ############################
latent_dimension = 16 # 128 # 
...
def load_train(train_path, sample_rate, label_class, num_images, BATCH_SIZE):
...
#                                                     shuffle=True,
#                                                     drop_last=False)
    train_data_loader, _ = utils.get_loaders(num_images, dataset='mnist', label_class=label_class, batch_size=BATCH_SIZE)
    return train_data_loader, num_images #imagenet_data.__len__()


def load_test(test_path, label_class, BATCH_SIZE, sample_rate):
    transform = torchvision.transforms.Compose([
        torchvision.transforms.Resize(256),
        torchvision.transforms.CenterCrop(256),
        torchvision.transforms.ToTensor(),
...
#                                                     pin_memory=True,
#                                                     shuffle=False)
    _, valid_data_loader = utils.get_loaders(num_images, dataset='mnist', label_class=label_class, batch_size=BATCH_SIZE)
...
def train(args, NORMAL_NUM,
          ...
          optimizer_g, optimizer_d, train_size, BATCH_SIZE):
    AUC_LIST = []
    ...
    train_path = './cifar10/train/airplane/' #.format(NORMAL_NUM)
    START_ITER = 0
    ...
    train_dataset_loader, _ = load_train(train_path, args.sample_rate, NORMAL_NUM, train_size, BATCH_SIZE)
    ...
    print('generator sigma: ', generator.sigma)
...
#             recorder.record(loss=l1_batch_wise, epoch=int(iteration / BATCH_SIZE),
        ...
        if iteration % int((train_size / BATCH_SIZE) * 15) == 0 or iteration == END_ITER - 1: # * 5
            is_end = True if iteration == END_ITER - 1 else False
            test_auc, AUC_LIST = validation(NORMAL_NUM, iteration, generator, discriminator, real_data, fake_data, is_end, AUC_LIST, END_ITER, BATCH_SIZE)
...
def validation(NORMAL_NUM, iteration, generator, discriminator, real_data, fake_data, is_end, AUC_LIST, END_ITER, BATCH_SIZE):
    discriminator.eval()
    generator.eval()
    # resnet.eval()
    y = []
    ...
    test_root = './cifar10/train/' #.format(NORMAL_NUM) #automobile/
    ...
    with torch.no_grad():
        for i in range(1): # range(len(list_test)): #
            ...
            valid_dataset_loader = load_test(test_path, NORMAL_NUM, BATCH_SIZE, sample_rate=1.)
...
if __name__ == "__main__":
    ...
    train_size_choices = [100] # 2,5,10,20,40,50,
    
    for num_images in train_size_choices:
        if(num_images < 30): BATCH_SIZE = int(num_images/2)
        elif(30 < num_images < 60): BATCH_SIZE = int(num_images/3)
        else:  BATCH_SIZE = int(num_images/6)
        
        print('---------------------------------------------------')
        print(str(num_images), ' = size of training dataset. START.')
        print('---------------------------------------------------')
        
        for i, key in enumerate(sorted(sub_category)):
            ...
            train_path = './cifar10/train/'#.format(NORMAL_NUM)
            test_root = './cifar10/test/'#.format(NORMAL_NUM)
            gt_root = './mvtec/{}/ground_truth/'.format(NORMAL_NUM)
    ...
    #         auc_file.close()
            ...
            generator = twoin1Generator256(32, latent_dimension=latent_dimension) #64
            discriminator = VisualDiscriminator256(32) #64
            ...
            train(args, NORMAL_NUM, generator, discriminator, optimizer_g, optimizer_d, num_images, BATCH_SIZE)
