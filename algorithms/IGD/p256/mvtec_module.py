...
class UpSampleConv(torch.nn.Module):
    ...
    def forward(self, x):
        output = x
    #    print('x shape in upsample is {}'.format(x.shape))
...
class ResidualBlock(torch.nn.Module):
    ...
    def forward(self, x):
    #    print('x shape in residual block is {}'.format(x.shape))
...
#
    def forward(self, x):
        output = x.contiguous()
        output = output.view(-1, 3, 256, 256) #224, 224) #
        output = self.conv1(output)
        output = self.rb1(output)
        output = self.rb2(output)
        output = self.rb3(output)
